...
def create_context_for_crons_html(context, cron_jobs, error_messages=None, draft_cron_jobs=None):
    ...
    cron_mappings = {
        cron_mapping['job_name']:
            {
                **cron_mapping,
                'active': cron_mapping['job_name'] in list(cron_jobs.keys()),
                "schedule": cron_jobs[cron_mapping['job_name']].schedule
                if cron_mapping['job_name'] in cron_jobs else ""
            }
        for cron_mapping in CRON_JOB_MAPPING.values()
    }
